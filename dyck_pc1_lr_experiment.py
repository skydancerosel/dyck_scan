#!/usr/bin/env python3
"""
LR scaling law analysis for Dyck depth-prediction.

Uses existing data from dyck_lr_sweep_results.pt and dyck_sweep_results/
to derive defect onset lead time scaling law.

Tests whether Dyck exhibits super-linear scaling:
  lead_time ~ grok_step^alpha, with alpha > 1
  => predictive window IMPROVES at slower learning rates.

Also computes PC1 trajectories from existing weight snapshots (default LR).
"""
...
SEED = 42
ACC_KEY = "test_acc"  # Dyck uses test_acc, not test_seq_acc
...
def find_grok_step_acc(records, acc_threshold=0.98, sustained=3):
    """Find first step where test_acc >= threshold for `sustained` consecutive evals."""
    count = 0
    first_step = None
    for r in records:
        if r.get(ACC_KEY, 0) >= acc_threshold:
            if count == 0:
                first_step = r["step"]
            count += 1
            if count >= sustained:
...
def load_all_data():
    ...
    for seed in [42, 137, 2024]:
        ...
        if fpath.exists():
            ...
            # Figure out which LR these were trained at
            cfg = data.get("cfg", {})
            lr_val = cfg.get("LR", 1e-4) if isinstance(cfg, dict) else 1e-4
            key = (lr_val, seed)
            if key in all_data:
...
def plot_lead_scaling(scaling_data):
    ...
    fig.suptitle("Dyck: Defect Onset Lead Time Scaling Law",
                 fontsize=14, fontweight='bold')
    fig.tight_layout()
    fig.savefig(OUT_DIR / "figPC1_dyck_lr_lead_scaling.png", dpi=150,
                bbox_inches="tight")
    plt.close(fig)
    print(f"  saved figPC1_dyck_lr_lead_scaling.png")
...
def plot_lead_scaling_multiseed(all_data):
    ...
    fig.suptitle("Dyck: Defect Onset Lead Time Scaling (all seeds)",
                 fontsize=14, fontweight='bold')
    fig.tight_layout()
    fig.savefig(OUT_DIR / "figPC1_dyck_lr_lead_scaling_multiseed.png", dpi=150,
                bbox_inches="tight")
    plt.close(fig)
    print(f"  saved figPC1_dyck_lr_lead_scaling_multiseed.png")
...
def plot_hero(all_data):
    ...
    for lr in all_lrs:
        ...
        test_accs = [r.get(ACC_KEY, 0) for r in recs]
    ...
    ax2.set_ylim(0, 1.05)
    ax.set_title("Dyck: Defect Predicts Grokking across Learning Rates",
    ...
    fig.tight_layout()
    fig.savefig(OUT_DIR / "figPD2_dyck_pc1_lr_hero.png", dpi=150,
                bbox_inches="tight")
    plt.close(fig)
    print(f"  saved figPD2_dyck_pc1_lr_hero.png")
...
def plot_pc1_trajectory(attn_logs, grok_step=None, lr=None, seed=None):
    ...
    if grok_step and turnover:
        ...
        ax.set_title(f"Dyck PC1 Trajectory (lr={lr:.0e}, seed={seed}, lead={lead})")
    else:
        ax.set_title(f"Dyck PC1 Trajectory (lr={lr:.0e}, seed={seed})")
    ...
    fig.tight_layout()
    fname = f"figPC1_dyck_trajectory_lr{lr:.0e}_s{seed}.png"
...
if __name__ == "__main__":
    print("="*60)
    print("Dyck: LR Scaling Law Analysis")
    print("="*60)
    ...
    print(f"\n  Per-run analysis (grok defined as first {ACC_KEY} >= 0.98, sustained 3):")
    ...
    # -- PC1 trajectory (weight snapshots available) --
    print(f"\n{'='*60}")
    print("PC1 TRAJECTORY (weight snapshots from dyck_sweep_results/)")
    print(f"{'='*60}")

    pc1_data = {}
    for seed in [42, 137, 2024]:
        for lr_val in sorted(set(k[0] for k in all_data.keys())):
            key = (lr_val, seed)
            if key in all_data and "attn_logs" in all_data[key] and all_data[key]["attn_logs"]:
                ...
                pc1 = plot_pc1_trajectory(all_data[key]["attn_logs"],
                                          grok_step=grok_step, lr=lr_val, seed=seed)
                pc1_data[(lr_val, seed)] = pc1
    ...
    if len(scaling_points) >= 2:
        ...
        # Compare with SCAN
        print(f"\n  SCAN result: alpha ~ 1.17")
        comparison = "similar to" if abs(beta - 1.17) < 0.3 else "different from"
        print(f"  Dyck: alpha ~ {beta:.2f} ({comparison} SCAN)")
